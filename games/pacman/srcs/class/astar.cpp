...
std::vector<Pos> Astar::findPath(const Pos &start, const Pos &goal)
{
	...
	while (!_openSet.empty()) {
		...
		for (const auto &_dir : _dirs) {
			tmp = current->pos + _dir;
			if (tmp.x < 0 || tmp.y < 0 ||
				tmp.x >= _cols || tmp.y >= _rows ||
				(_map[tmp.y][tmp.x] == '#' ||  (_map[tmp.y][tmp.x] >= 'E' && _map[tmp.y][tmp.x] <= 'Z')))
				continue;
			neighbor = &_grid[tmp.y][tmp.x];
			if (isInList(_closedSet, *neighbor))
				continue;
			tempG = static_cast<int>(current->g + 1);
