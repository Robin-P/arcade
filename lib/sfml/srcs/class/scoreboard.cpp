...
std::string u_sfml::getScore(std::string &game_name)
{
...
// void u_sfml::setScore(std::string &game_name)
// {
// 	std::string path = SCOREBOARD + game_name + "/" + SCORE_FILE;
// 	std::ifstream file(path);
...
// 	if (!file.is_open())
// 		throw arcade::Exception("LIB SFML", path + " is missing.");
// 	file << "name: " << this->prompt << std::endl
// 	     << "score: " << this->score << std::endl;
// }

void u_sfml::displayScoreboard(const std::string &font_name, const std::string &game_selected,
...
void u_sfml::createScoreboard(std::map<std::string, void *> &game_lib_name)
{
