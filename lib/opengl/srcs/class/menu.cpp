...
void u_opengl::initMenu(std::map<std::string, void *> &game_lib_name,
...
arcade::menuLeave u_opengl::leaveMenu()
{
	if (!this->GamePathName.empty()) {
		this->cleanRessources();
		if (!GamePathName.empty()) {
			this->createSound("start", "./audio/sound/" +
...
arcade::menuLeave u_opengl::menu(std::map<std::string, void *> &game_lib_name,
			 std::map<std::string, void *> &graphics_lib_name)
{
	sf::Event event{};
