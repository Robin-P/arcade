...
#ifndef LIB_OPENGL_CLASS_HPP_
#define LIB_OPENGL_CLASS_HPP_
...
namespace arcade {
	...
	public:
		...
		void opengl_event(sf::Event &event);
		...
		void displayPrompt();
		//OPENGL 3D
		void drawCube(float x, float y, float size, int r, int g, int v, size_t map_size);
		void drawSquare(float x, float y, float size, int r, int g, int v, size_t map_size);
		void draw3dMap(std::vector<std::string> &array);
		void clearOpengl();
		void refreshOpengl();
	public:
...
using u_opengl = arcade::Opengl;
#endif /* !LIB_OPENGL_CLASS_HPP_ */
